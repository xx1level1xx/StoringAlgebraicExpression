...
int main(){
	...
	variables="b,y";
	string operators="+,-,*,/,^,=";
	//isExp well formed, missplaced operators (repeating, start with an operator or end with an operator, end of the name of a special function), dangeling paranthesis, wrong variables //combine ahhhhhhhhhhhh
	//Missplaced operators
	int count=0;
	...
	//after the special functions
	bool correct=true;
	for(int i=0;i<exp.size();i++){
		if(i+2<exp.size()&&exp.substr(i,3)=="abs"){
			if(i+3>=exp.size()||exp[i+3] != '('){
				correct = false;
			}
		}
